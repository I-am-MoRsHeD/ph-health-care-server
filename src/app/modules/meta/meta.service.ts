...
const fetchDashboardMetaData = async (user: IPayloadProps) => {
    let metaData;
    switch (user.role) {
        case UserRole.ADMIN:
...
const getDoctorMetaData = async (user: IPayloadProps) => {
    ...
    const appointmentCount = await prisma.appointemnt.count({
        where: {
            doctorId: doctorData.id
        }
    });

    const patientCount = await prisma.appointemnt.groupBy({
        by: ['patientId'],
        _count: {
            id: true
        }
    ...
    const appointmentStatusDistribution = await prisma.appointemnt.groupBy({
        by: ['status'],
        _count: { id: true },
        where: {
            doctorId: doctorData.id
...
const getPatientMetaData = async (user: IPayloadProps) => {
...
const getBarChartData = async () => {
    const appointmentCountPerMonth = await prisma.$queryRaw`
        ...
        FROM "appointments" -- map a je name dewa ache oita
        GROUP BY month
        ORDER BY month ASC
    `
